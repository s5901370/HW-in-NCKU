...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T> *p = &_recycleList[m] ,*p1;
      while(p){
         if(p->_arrSize == n)
            return p;
         p1 = p;
   ...
   T* getMem(size_t t) {
      ...
      MemRecycleList<T> * p = getMemRecycleList(n);
      ...
      if(p->_first){
         ...
         // cout<<"A"<<endl;       
         // cout<<_activeBlock->getRemainSize()<<endl;                    //
         ...
         cout << "Memory acquired... " << ret << endl;
            ...
            if(_activeBlock->getRemainSize() >= S){
               // cout<<"A"<<endl;                 //
               size_t rem = downtoSizeT(_activeBlock->getRemainSize());
               ...
               // cout<<rn<<" "<<getMemRecycleList(rn)<<endl;        //
   ...
   size_t getNumBlocks() const {
      ...
      if(!_activeBlock)
         return 0;
      size_t count = 1;
      MemBlock<T>* p =_activeBlock->_nextBlock;
      while(p){
         ++count;
         p = p->_nextBlock;
      }   
