...
public:
   ...
   void newObjs(size_t n) {
      // TODO
   	for(size_t i = 0; i < n; ++i){
   		// cout<<endl;
   		MemTestObj *p = new MemTestObj;//cout<<"b"<<endl;
   		if(p) _objList.push_back(p);
   	}
   }
   ...
   void newArrs(size_t n, size_t s) {
      // TODO
    for(size_t i = 0; i < n; ++i){
   		MemTestObj *p = new MemTestObj[s];

   		if(p) _arrList.push_back(p);
   	}
   }
