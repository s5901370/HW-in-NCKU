			...
			if(dbtbl){
				...
				for(size_t i = 0;i<dbtbl.nCols();++i){
					if(i+1 < options.size()){
						...
						else if(myStr2Int(options[i+1],c))
							temp.addData(c);
						else
							cerr << "Error: "<<'"'<<options[i+1]<<'"'<<" is not an integer!!";
					}
					else{
						temp.addData(INT_MAX);	
					}
   ...
   // TODO...
	...
	if(options.size()>0){
		if(!myStrNCmp("-Row",options[0] ,2))
			...
			doCol = true;
		else if(!myStrNCmp("-Table",options[0] ,2))
			doTbl = true;
		else if(!myStrNCmp("-Summary" , options[0], 2))
			doSum = true;
		else if(!myStr2Int(options[0], r)){
			cout<<"A"<<endl; //
			return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);
		}
	}
