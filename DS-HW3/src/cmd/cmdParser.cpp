...
{
   // TODO...CmdMap::iterator it = _cmdMap.begin();it!=_cmdMap.end();++it
...
CmdParser::parseCmd(string& option)
{
   ...
   string token ;
   size_t i = myStrGetTok(str,token);
   // cout<<i<<" "<<token<<endl;
...
CmdParser::listCmd(const string& str)
{
   ...
   if(i==str.size()){
      ...
      for(CmdMap::iterator it = _cmdMap.begin();it!=_cmdMap.end();++it){
         string s = it->first;
         string s1 = it->second->getOptCmd();
         cout<<setw(12)<<left<<s+s1;
         count++;
         if(count%5==0){
            cout<<endl;
            count = 0;
   ...
   ///sec 2
      ...
      string token;
      size_t t;
      if((t=myStrGetTok(tail,token)) != string::npos){
         if(!getCmd(token)){
            mybeep();
            return;
         }
         if(_tabPressCount == 1){
            ...
            for(CmdMap::iterator it = _cmdMap.begin();it!=_cmdMap.end();++it){
               string s = it->first;
               string s1 = it->second->getOptCmd();
               if(token.size()>(s+s1).size())
                  return ;
               if(!myStrNCmp(s+s1,token,token.size())){
                  copy.push_back(s+s1); 
                  e = it->second;
               }
            }
            ...
            if( tail[tail.size()-1] == ' '){ 
               ...
               int count =1;
               cout<<endl;
               for(size_t j =0,n = file.size();j < n;++j){
                  if(count%6==0){
                     cout<<endl;
                     count = 1;
                  }
                  count++;
                  cout<<setw(16)<<left<<file[j];
               }
               reprintCmd();
            }
            else if(t < tail.size()-1){//t==size()-2
               vector <string> file;
               listDir(file,tail.substr(t+1,tail.size()-1-t),".");
               if(file.size()==1){
                  string ss = file[0].substr(1);
                  for(size_t j = 0,n = ss.size();j<n;++j)
                     insertChar(ss[j]);
                  insertChar(' ');
                  return ;
               }
                  ...
                  vector <int> max;
                  size_t g ;
                  for(size_t k=0,m = file.size()-1;k<m;++k){
                     for(size_t u = k+1,x = file.size();u < x;++u){
                        g=1;
                        while(g<file[k].size() && g<file[u].size() && (file[k][g]==file[u][g]))
                           g++; // g numbers are the same
                        max.push_back(g);
                     }
                  }
                  ...
                  g = max[0];
                  if(g>1){
                     size_t a = tail.size()-1-t;
                     for(size_t k=a;k < g;++k)
                        insertChar(file[0][k]);
                     reprintCmd();
                  }
                  else{ //g==0
      ...
      for(CmdMap::iterator it = _cmdMap.begin();it!=_cmdMap.end();++it){
         string s = it->first;
         string s1 = it->second->getOptCmd();
         if(tail.size()>(s+s1).size())
            return ;
         if(!myStrNCmp(s+s1,tail,tail.size()))
            copy.push_back(s+s1); 
      }
      if(!copy.empty()){
         if(copy.size()>1){
            int count = 1;
            cout<<endl;
            for(size_t j =0,n = copy.size();j < n;++j){
               if(count%6==0){
                  cout<<endl;
                  count = 1;
               }
               count++;
               cout<<setw(12)<<left<<copy[j];
            }
            reprintCmd();
            return ;
         }
...
CmdParser::getCmd(string cmd)
{
   ...
   // TODO...
   for(CmdMap::iterator it = _cmdMap.begin();it!=_cmdMap.end();++it){
      string s = it->first , s1 = it->second->getOptCmd();
      if(!myStrNCmp(s+s1,cmd,s.size())){ // s or s+s1??
         e = it->second;
      }
   }
   return e;
