...
extern Game *game;
int E_Elf :: mortal = 0;
E_Elf::E_Elf(int x,int y):step(10),attack_damage(5),c_Tower(false),hp(35){
    //set pos
    setPos(x,y);
    //set graph
    setPixmap(QPixmap(":/images/cute/e_elf.png"));
...
void E_Elf::move(){
    ...
    for(int i=0,n=colliding_items.size();i<n;i++){
        ...
        if(tower){
            ...
            tower->health->hurt(attack_damage-3);//tower be attacked
            tower->tower_be_damaged(attack_damage-3);
